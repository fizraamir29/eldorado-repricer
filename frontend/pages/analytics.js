...
export default function AnalyticsPage() {
  ...
  async function loadData(showSpinner = false) {
    ...
    try {
      ...
      const fetchedListings = listingsRes.data || [];
      const fetchedSummary = summaryRes.data || null;
      ...
      const ruleEntries = await Promise.all(
        fetchedListings.map(async (l) => {
          try {
            ...
            return [l.id, { enabled: false }];
          }
        })
      );
      const rulesMap = Object.fromEntries(ruleEntries);

      const listingsWithRules = fetchedListings.map(l => ({
        ...l,
        rule_enabled: rulesMap[l.id]?.enabled || false
      }));

      setListings(listingsWithRules);
